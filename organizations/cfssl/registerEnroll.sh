...
function peer_cert() {
    ...
    mkdir -p "organizations/peerOrganizations/$ORG.aria.com.np/ca"
    mkdir -p "organizations/peerOrganizations/$ORG.aria.com.np/msp/cacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.aria.com.np/msp/tlscacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.aria.com.np/peers"
    mkdir -p "organizations/peerOrganizations/$ORG.aria.com.np/tlsca"

    CERT_DIR=organizations/peerOrganizations/$ORG.aria.com.np

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-peer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.$ORG.aria.com.np-cert.pem"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/ca/ca.$ORG.aria.com.np-cert.pem"
...
function orderer_cert() {
    TYPE=$1 #orderer user
    USER=$2 #orderer.aria.com.np

    mkdir -p organizations/ordererOrganizations/aria.com.np/ca
    mkdir -p organizations/ordererOrganizations/aria.com.np/msp/cacerts
    mkdir -p organizations/ordererOrganizations/aria.com.np/msp/tlscacerts
    mkdir -p organizations/ordererOrganizations/aria.com.np/orderers
    mkdir -p organizations/ordererOrganizations/aria.com.np/tlsca

    CERT_DIR=organizations/ordererOrganizations/aria.com.np

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-orderer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.aria.com.np-cert.pem"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/cacerts/"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/tlscacerts/tlsca.aria.com.np-cert.pem"
...
function generate_orderer_certs() {
    ...
    for DIR in cacerts keystore signcerts tlscacerts; do
        mkdir -p "organizations/ordererOrganizations/aria.com.np/orderers/$USER/msp/$DIR"
    done

    mkdir -p "organizations/ordererOrganizations/aria.com.np/orderers/$USER/tls"
    ...
    cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/orderers/$USER/msp/tlscacerts/tlsca.aria.com.np-cert.pem"
